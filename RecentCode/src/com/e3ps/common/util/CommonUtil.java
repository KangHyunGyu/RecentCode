...
//			}
    ...
    public static Map<String, Object> getHmacInKey(String companyCd, String empId, String apiTime, String signiture) {
    	...
    	try {
    		...
    		if(companyCd != null && empId != null && apiTime != null) {
    			String userInfoString = companyCd+empId+apiTime; // 조합번호
            	String apiKey = ConfigImpl.getInstance().getString("SLO.APIKEY.CODE");
            	String algorithm = "HmacSHA256";

            	SecretKeySpec secretKey = new SecretKeySpec(apiKey.getBytes("utf-8"), algorithm);

            	Mac hasher = Mac.getInstance(algorithm);
            	hasher.init(secretKey);

            	byte[] hash = hasher.doFinal(userInfoString.getBytes());
