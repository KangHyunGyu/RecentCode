...
public class ObjectUtil
{
    ...
    {
        LOGGER.info("VersionUtil revise()-------------------------------------------------");
        LOGGER.info("arg lifecycle - " + lifecycle);
        if (lifecycle == null)
            lifecycle = ((LifeCycleManaged) obj).getLifeCycleName();
        ...
        obj = VersionControlHelper.service.newVersion(obj, vi, VersionControlHelper.firstIterationId(obj));
        FolderHelper.assignLocation((FolderEntry) obj, folder);
        LOGGER.info("LifeCycleHelper.service.getLifeCycleTemplate--"
	...
	public static RevisionControlled getFirstObj(WTDocument doc) throws VersionControlException, WTPropertyVetoException, WTException {
		RevisionControlled rc = null;
        QueryResult qr = VersionControlHelper.service.allIterationsOf((Mastered)doc.getMaster());
        while (qr.hasMoreElements())
        {
            RevisionControlled obj = ((RevisionControlled) qr.nextElement());
            if (obj.getVersionIdentifier().getSeries().getValue().equals("A") && obj.getIterationIdentifier().getSeries().getValue().equals("1")) {
                rc = obj;
                break;
            }
        }
        if (rc != null)
            return (RevisionControlled) rc;

        return null;
	}
	
	public static RevisionControlled getFirstObj(WTPart part) throws VersionControlException, WTPropertyVetoException, WTException {
		RevisionControlled rc = null;
        QueryResult qr = VersionControlHelper.service.allIterationsOf((Mastered)part.getMaster());
        while (qr.hasMoreElements())
        {
            RevisionControlled obj = ((RevisionControlled) qr.nextElement());
            if (obj.getVersionIdentifier().getSeries().getValue().equals("A") && obj.getIterationIdentifier().getSeries().getValue().equals("1")) {
	...
	public static RevisionControlled getFirstObj(EPMDocument doc) throws VersionControlException, WTPropertyVetoException, WTException {
		RevisionControlled rc = null;
        QueryResult qr = VersionControlHelper.service.allIterationsOf((Mastered)doc.getMaster());
        while (qr.hasMoreElements())
        {
