...
public class TemplateLoader_Before implements RemoteAccess, Serializable {
	...
	public static void main(String[] args) throws Exception {
		...
		new TemplateLoader_Before().load(sFilePath);
		...
		try {
			...
			for(int i=3; i<rows; i++) {
				if(checkLine(sheet.getRow(i), 0) || checkLine(sheet.getRow(i), 1) || checkLine(sheet.getRow(i), 2)) {
					...
					String preTask = getContent(cell, 6).trim();//선행 태스크
					String outputNames = getContent(cell, 7).trim();//산출물
					String outputPaths = getContent(cell, 8).trim();//산출물 경로
					String tOutputType = getContent(cell, 9).trim();//산출물타입
					String outputStep = getContent(cell, 10).trim();//산출물 인증타입
					String roles = getContent(cell, 11).trim();//Role
