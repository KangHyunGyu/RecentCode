...
public class TemplateLoader implements RemoteAccess, Serializable {
	...
	public static void main(String[] args) throws Exception {
		...
		String sFilePath = sWtHome + "\\loadFiles\\e3ps\\WorldexMassProductTemplateForm8.xls";
			...
			for(int i=3; i<rows; i++) {
				if(checkLine(sheet.getRow(i), 0) || checkLine(sheet.getRow(i), 1) || checkLine(sheet.getRow(i), 2)) {
					...
					if(outputNames != null && outputNames.length() > 0) {
						...
						for(NumberCodeData docData : CodeHelper.manager.getNumberCodeList("DOCCODETYPE")) {
							if(outputNames.equals(docData.getName())){
								docParam.put("key", docData.getCode());
								break;
							}else {
								docParam.put("key", "D009");
							}
						}
