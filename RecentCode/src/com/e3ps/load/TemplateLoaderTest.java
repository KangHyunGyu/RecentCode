...
public class TemplateLoaderTest implements RemoteAccess, Serializable {
	...
	public static void main(String[] args) throws Exception {
		...
		String sFilePath = sWtHome + "\\loadFiles\\e3ps\\ProductTemplateForm.xls";
		if (args != null && args.length > 0 && args[0].trim().length() > 0) {
			sFilePath = args[0];
		}
		setUser("wcadmin", "wcadmin");
		new TemplateLoader().load(sFilePath);
	}
	...
	public void load(String sFilePath) throws RemoteException, InvocationTargetException {
		...
		try {
			...
			String projecttype = getContent(cell, 2).trim();
			...
			for(NumberCodeData data : CodeHelper.manager.getNumberCodeList("PROJECTTYPE")) {
				if(projecttype.equals(data.getName())){
					param.put("key", data.getCode());
					break;
				}else {
					param.put("key", "E");
				}
			...
			templateMap.put("projecttype", param.get("key"));
			...
			for(int i=3; i<rows; i++) {
				if(checkLine(sheet.getRow(i), 0) || checkLine(sheet.getRow(i), 1) || checkLine(sheet.getRow(i), 2)) {
					...
					String outputDoc = getContent(cell, 7).trim();//산출물 분류
					String outputType = getContent(cell, 8).trim();//산출물 인증타입
					...
					if(preTask != null && preTask.length() > 0) {
						...
						ETask prtl = taskSavedHash.get(preTask);

						PrePostLink link = PrePostLink.newPrePostLink(prtl, task);
					...
					if(outputNames != null && outputNames.length() > 0) {
						...
						for(DocCodeType docData : DocHelper.manager.getDocCodeTypeList(outputDoc)) {
							
							if(outputDoc.equals(docData.getName())){
								if(!taskName.equals(preTask)) {
									docParam.put("key", docData.getCode());
									break;
								}
							}
						}
						...
						for(int o=0; o<outputName.length; o++) {
							...
							outputMap.put("docCodeType", docParam.get("key"));
							outputMap.put("outputType", outputType);
...
//			trx.rollback();
	...
	public ETask createTask(Map<String, Object> reqMap) throws Exception {
		...
		try {
			...
			if (parent instanceof EProjectNode || parent instanceof EProjectTemplate) {
