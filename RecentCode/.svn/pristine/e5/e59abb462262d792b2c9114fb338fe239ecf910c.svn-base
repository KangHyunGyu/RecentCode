...
function openSearchECRPopup(id, type) {
	
	var url = getURLString("/change/searchEcrPopup");
...
function openRoleUserPopup(id, oid) {
	
	var url = getURLString("/project/searchRoleUserPopup");

	url += "?id=" + id;
	url += "&oid=" + oid;
	
	openPopup(url,"openRoleUserPopup", 1200, 700);
}

function openDistributeDocPopup(id, state) {
	
	var url = getURLString("/distribute/searchDistributeDocPopup");

	url += "?id=" + id +"&state=" + state;
	
	openPopup(url,"searchDistributeDocPopup", 1200, 700);
}

function deleteDistributeDoc(id){
	$("#" + id).val("");
	$("#" + id).empty();
	$("#" + id).trigger("change");
}
