 ...
 * @desc setOrderBy (Choi Seunghwan,2005.10.06)
 */
...
public class SearchUtil
{
    ...
    /**
	 * ControlBranch 의 모자관계를 사용하여 LatestRevision 을 검색한다
	 * @param qs
	 * @param targetClass
	 * @param idx
	 * @throws WTException
	...
	public static void addIBAConditionLike(QuerySpec _query, Class _target, int _idx, String ibaName, String ibaValue) throws IBADefinitionException,
		...
		if (aview != null) {
			   ...
			   int idx = _query.appendClassList(StringValue.class, false);
			   SearchCondition sc =
			         new SearchCondition(new ClassAttribute(StringValue.class, "theIBAHolderReference.key.id"), "=", new ClassAttribute(_target,
	...
	public static void addIBAConditionString(QuerySpec _query, Class _target, int _idx, String ibaName, String ibaValue) throws WTException, RemoteException, WTPropertyVetoException{
		...
		if (aview != null) {
		   ...
		   StringSearch stringSearch = new StringSearch("value");
			...
			_query.appendWhere(stringSearch.getSearchCondition(StringValue.class),
				new int[] { idx });
		}
	}
    ...
    private static String[][] wildcardInfos = new String[][] {
		{"*", "\\*", "%"},
		{"?", "\\?", "_"}
	};
	public static void appendWildcardWhere(
				QuerySpec qs, Class targetClass, String attribute, String value, int index) 
			throws WTException {
		...
		if (qs.getConditionCount() > 0) qs.appendAnd();
		qs.appendWhere(new SearchCondition(targetClass, attribute, operator, searchText),
			new int[] { index }
		);
	}
	

	public static QuerySpec getKeywordQuery(Class targetClass,QuerySpec mainQuery,String searchValue,String[] ibaNames, String alias) throws WTPropertyVetoException, WTException, RemoteException{
	       ...
	       QuerySpec qs = new QuerySpec();
			...
			qs.appendWhere(new SearchCondition(StringValue.class, "theIBAHolderReference.key.id",
					targetClass, "thePersistInfo.theObjectIdentifier.id"), new int[] { _idx, idx });
			qs.appendAnd();
			...
			qs.appendWhere(stringSearch.getSearchCondition(targetClass),
				new int[] { idx });
			...
			SearchCondition correlatedJoin = new SearchCondition(targetClass, WTAttributeNameIfc.ID_NAME, targetClass, WTAttributeNameIfc.ID_NAME);
			qs.appendAnd();
	   ...
	   private static void addIBACondition(QuerySpec query, int idx,int _idx, AttributeDefDefaultView aview , String value) throws WTException, RemoteException, WTPropertyVetoException{
			
			if (aview != null) {
				query.appendWhere(
						new SearchCondition(StringValue.class, "definitionReference.key.id", "=", aview.getObjectID().getId()), new int[] { _idx });
				query.appendAnd();
				StringSearch stringSearch = new StringSearch("value");
				stringSearch.setValue(value.trim());
				query.appendWhere(stringSearch.getSearchCondition(StringValue.class),
					new int[] { _idx });
			}
		}
	   
	   public static void addIBACondition(QuerySpec qs,Class targetClass, int idx, String viewName , String value) throws WTException, RemoteException, WTPropertyVetoException{
			
		   AttributeDefDefaultView aview = 
					IBADefinitionHelper.service.getAttributeDefDefaultViewByPath(viewName);
		   ...
		   if (aview != null) {
				...
				StringSearch stringSearch = new StringSearch("value");
				stringSearch.setValue(value.trim());
				qs.appendWhere(stringSearch.getSearchCondition(StringValue.class),
						new int[] { _idx });
		   }
		}
	   
	   public static void addIBADotCondition(QuerySpec query,Class targetClass, int idx, String viewName , String value) throws WTException, RemoteException, WTPropertyVetoException{
			
		   AttributeDefDefaultView aview = IBADefinitionHelper.service
					.getAttributeDefDefaultViewByPath(viewName);
		   
			if (aview != null) {
				
				int _idx = query.appendClassList(StringValue.class, false);
				query.appendWhere(new SearchCondition(StringValue.class,
						"theIBAHolderReference.key.id", targetClass,
						"thePersistInfo.theObjectIdentifier.id"),
						new int[] { _idx, idx });
				query.appendAnd();
				query.appendWhere(
						new SearchCondition(StringValue.class,
								"definitionReference.key.id",
								SearchCondition.EQUAL, aview.getObjectID()
										.getId()), new int[] { _idx });
				query.appendAnd();
				...
				query.appendWhere(
						stringSearch.getSearchCondition(StringValue.class),
						new int[] { _idx });
				...
				if (!value.toUpperCase().trim().endsWith("*")) {
					...
					stringSearch.setValue(value.toUpperCase().trim()
							+ ",%");
					query.appendWhere(stringSearch
							.getSearchCondition(StringValue.class),
							new int[] { _idx });
				}
				if (!value.toUpperCase().trim().startsWith("*")) {
					query.appendOr();
					stringSearch = new StringSearch("value");
					stringSearch.setValue("%,"
	   ...
	   public static void addUserFullNameCondition(QuerySpec query,Class targetClass, int idx, String queryKey , String value) throws WTException, RemoteException, WTPropertyVetoException{
			
		   int userIdx = query.appendClassList(WTUser.class,false);
			
			query.appendWhere(
					new SearchCondition(targetClass,
							queryKey, WTUser.class,
							"thePersistInfo.theObjectIdentifier.id"),
					new int[] { idx, userIdx });
			query.appendAnd();
			...
			stringSearch.setValue(value.trim());
			query.appendWhere(
					stringSearch.getSearchCondition(WTUser.class),
					new int[] { userIdx });
		}
	   
	   
	  
	   public static void appendOrderBy(QuerySpec query,Class targetClass, String sortColumn, int idx, boolean desc) throws WTException, RemoteException, WTPropertyVetoException{
		   ...
		   if (sortColumn.startsWith(IBA_PREFIX)) {
				...
				if(aview!=null){
					...
					SearchCondition sc = new SearchCondition(StringValue.class, "theIBAHolderReference.key.id",
							targetClass, "thePersistInfo.theObjectIdentifier.id");
					...
					if(codeType!=null){
						appendCodeOrderBy(query,StringValue.class,codeType,StringValue.VALUE, _idx, desc);
					}else{
						//nvl(A4.value,' ')
			...
			}else if (sortColumn.startsWith(USER_PREFIX)) {
				
				sortColumn = sortColumn.substring(USER_PREFIX.length());
				...
				ClassAttribute ca = new ClassAttribute(targetClass,
						sortColumn);
				ClassAttribute ca2 = new ClassAttribute(WTUser.class,
						"thePersistInfo.theObjectIdentifier.id");
				...
				query.appendAnd();
				query.appendWhere(new SearchCondition(ca2, "=", ca3), new int[] { idx_user,
						idx_people });
				
				ClassAttribute classattribute = new ClassAttribute(People.class, People.NAME);
		        classattribute.setColumnAlias("sort0");
		        int[] fieldNoArr = { idx_people };
		        query.appendSelect(classattribute, fieldNoArr, false);
		        OrderBy orderby = new OrderBy(classattribute, desc, null);
				...
				String codeType = null;
			   	...
			   	if(codeType!=null){
					appendCodeOrderBy(query,targetClass,codeType,sortColumn, idx, desc);
				}else{
	   ...
	   public static void appendCodeOrderBy(QuerySpec query,Class targetClass,String codeType, String joinColumn, int idx, boolean desc) throws WTException, RemoteException, WTPropertyVetoException{

		   	if (query.getConditionCount() > 0){
				query.appendAnd();
			}
		   
			int idx_code = query.appendClassList(NumberCode.class, false);
			SearchCondition sc = new SearchCondition(NumberCode.class,NumberCode.CODE,targetClass,joinColumn);
			sc.setOuterJoin(SearchCondition.LEFT_OUTER_JOIN);
			query.appendWhere(sc,new int[]{idx_code,idx });
			query.appendAnd();
			sc = new SearchCondition(NumberCode.class,NumberCode.CODE_TYPE,"=",codeType);
			sc.setOuterJoin(SearchCondition.LEFT_OUTER_JOIN);
			query.appendWhere(sc,new int[]{idx_code});
			ClassAttribute attribute = new ClassAttribute(NumberCode.class, NumberCode.NAME);
			//nvl(A4.value,' ')
			SQLFunction function = SQLFunction.newSQLFunction(SQLFunction.NULL_VALUE);
			function.setArgumentAt((ColumnExpression) attribute, 0);
			function.setArgumentAt(new ConstantExpression(" "), 1);
			function.setColumnAlias("sort0");
			query.appendSelect(function, new int[]{idx_code}, false);
	        OrderBy orderby = new OrderBy(function, desc, null);
	        query.appendOrderBy(orderby,  new int[]{idx_code});
	   }
	   
	   
	   public static void appendSubQuery(QuerySpec mainQuery,QuerySpec subQuery,int mainIdx,int subIdx,Class targetClass) throws WTPropertyVetoException, WTException, RemoteException{
			...
			mainQuery.appendWhere(new ExistsExpression(subQuery),
					new int[] { mainIdx });
		}
	   
	   public static ClassAttribute getQK(Class targetClass, String columnName) throws QueryException, WTIntrospectionException {

		   
		   ColumnDescriptor[] descriptors = WTIntrospector.getDatabaseInfo(targetClass.getName()).getBaseTableInfo().getColumnDescriptors();

		   String queryKeyName = "";
		   
			for (int nIndex = 0 ; nIndex < descriptors.length ; nIndex++){
				String sColName = descriptors[nIndex].getColumnName();
				if(sColName.equalsIgnoreCase(columnName)) {
					queryKeyName = descriptors[nIndex].getName();
					break;
				}
			}	
			
			if(queryKeyName.length() != 0) {
				return new ClassAttribute(targetClass, queryKeyName);
			}
