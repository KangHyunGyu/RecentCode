...
var columnLayout = [
	...
	{ dataField : "request_user_name",		headerText : "${e3ps:getMessage('견적 요청자 이름')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "request_user_id",		headerText : "${e3ps:getMessage('견적 요청자 사번')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchEstimateInfoScrollAction"));
	...
	<div class="pro_table mr30 ml30">
		<form name="searchForm" id="searchForm">
			<table class="mainTable">
				...
				<tbody>
					<tr>
						<th scope="col">${e3ps:getMessage('견적서 번호')}</th>
						<td><input type="text" id="request_number" name="request_number" class="w100"></td>
						<th scope="col">${e3ps:getMessage('협력업체 코드')}</th>
						<td><input type="text" id="company_id" name="company_id" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('견적 요청자 이름')}</th>
						<td><input type="text" id="request_user_name" name="request_user_name" class="w100"></td>
						<th scope="col">${e3ps:getMessage('견적 요청자 사번')}</th>
						<td><input type="text" id="request_user_id" name="request_user_id" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('견적 요청자 이메일')}</th>
						<td><input type="text" id="request_user_email" name="request_user_email" class="w100"></td>
						<th scope="col">${e3ps:getMessage('견적서 상태')}</th>
						<td>
							<select class="w30" id="estimate_state" name="estimate_state">
								<option value="">${e3ps:getMessage('전체')}</option>
								<option value="N">${e3ps:getMessage('신규')}</option>
								<option value="F">${e3ps:getMessage('확정')}</option>
							</select>
						</td>
					</tr>
					<tr>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<c:if test="${e3ps:isSyncBtn()}">
				<button type="button" class="s_bt03" onclick="sync('syncERPEstimate');">${e3ps:getMessage('동기화')}</button>
			</c:if>
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPEstimate');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
