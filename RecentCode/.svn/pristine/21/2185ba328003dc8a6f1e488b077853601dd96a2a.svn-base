...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchPurchaseInfoScrollAction"));
			...
			<table class="mainTable">
				...
				<tbody>
					...
					<tr>
						...
						<td><input type="text" id="po_user_name" name="po_user_name" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('PO 담당자 사번')}</th>
						<td><input type="text" id="po_user_id" name="po_user_id" class="w100"></td>
						<th scope="col">${e3ps:getMessage('PO 담당자 e-mail')}</th>
						<td><input type="text" id="po_user_email" name="po_user_email" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('발주서 상태')}</th>
						<td><input type="text" id="purchase_state" name="purchase_state" class="w100"></td>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<c:if test="${e3ps:isSyncBtn()}">
				<button type="button" class="s_bt03" onclick="sync('syncERPPurchase');">${e3ps:getMessage('동기화')}</button>
			</c:if>
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPPurchase');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
