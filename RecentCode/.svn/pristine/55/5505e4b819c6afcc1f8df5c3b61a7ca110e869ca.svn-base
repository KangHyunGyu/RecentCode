				...
				<table class="mainTable">
					...
					<tbody>
						<tr>
							...
							<td class="calendar">
								<input type="text" class="datePicker w50" name="startDate" id="startDate" value="${project.startDate}" readonly/>
							</td>
						</tr>
					</tbody>
				</table>
...
function saveDate(){
	...
	$("#editForm").attr("action",getURLString("/project/modifyProjectStartDateAction"));
...
// 		}
		if(opener.window.updateProjectData) {
			opener.window.updateProjectData(data.project);
		}
...
// 	if(!checkDate("${startDate}",$("#task.pjtStartDate").val())){
// 		openNotice("${e3ps:getMessage('태스크 실제 시작일 보다 빠를 수 없습니다.')}");
// 		return;
// 	}
