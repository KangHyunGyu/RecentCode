 ...
 *
 * <p>
 * Use the <code>newEOutput</code> static factory method(s), not the <code>EOutput</code>
...
@GenAsPersistable(
   ...
   @GeneratedProperty(name="attachFile", type=boolean.class),
   ...
   @GeneratedForeignKey(name="OutputDocumentLink", myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="document", type=wt.doc.WTDocument.class),
      myRole=@MyRole(name="output")),
   @GeneratedForeignKey(name="StepOutputLink", myRoleIsRoleA=false,
      foreignKeyRole=@ForeignKeyRole(name="step", type=com.e3ps.project.OutputTypeStep.class),
      myRole=@MyRole(name="output"))
   })
public class EOutput extends _EOutput {
    ...
    **/
   public static EOutput newEOutput()
            throws WTException {

      EOutput instance = new EOutput();
      instance.initialize();
      return instance;
   }
