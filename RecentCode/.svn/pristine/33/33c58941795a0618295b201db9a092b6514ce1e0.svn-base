...
<div class="pop5">
	<!-- top -->
	<div class="top">
		<h2>${e3ps:getMessage('프로젝트 산출물 수정')}</h2>
	...
	<div class="pl25 pr25">
		<div class="seach_arm2 pt10 pb5">
			...
			<div class="rightbt">
				<button type="button" class="s_bt03" onclick="javascript:updateOutput()">${e3ps:getMessage('수정')}</button>
				<button type="button" class="s_bt03" onclick="javascript:deleteOutput()">${e3ps:getMessage('삭제')}</button>
			</div>
		</div>
		<form name="modifyOutputForm" id="modifyOutputForm" method="post">
				...
				<table class="mainTable">
					...
					<tbody>
						<tr>
							...
							<td>
								<input class="w100" type="text" id="name" name="name" value="${output.name}">
							</td>
						</tr>
<!-- 						<tr> -->
						...
						<tr> 
							...
							<td>
								<div style="position:relative">
									<input type="text" class="w50" id="locationDisplay" name="locationDisplay" value="${output.location}" disabled>
									...
									<div id="folderList" style="display:none;position:absolute;z-index:999">
										...
										<jsp:include page="${e3ps:getIncludeURLString('/common/include_folderTree')}" flush="true">
											...
											<jsp:param name="formId" value="modifyOutputForm"/>
											<jsp:param name="rootLocation" value="/Default/Document"/>
											<jsp:param name="gridHeight" value="180"/>
											<jsp:param name="location" value="${output.location}"/>
										</jsp:include>
										<!-- //tree -->
									</div>
								</div>
						...
						<tr>
							...
							<td class="pt5">
								<div class="textarea_autoSize">
									<textarea name="description" id="description">${output.description}</textarea>
								</div>
							</td>
						</tr>
					</tbody>
...
$(document).ready(function(){
	//lifecycle list
// 	getOutputTypeList();
// 	$("#outputType").val("${output.docType}");
// 	selectOutputType("#outputType");
// 	$("#outputStep").val("${outputStepOid}");
// 	selectOutputStep("#outputStep");
// 	$("#outputChildStep").val("${outputChildStepOid}");
});
...
function updateOutput(){
	...
	$("#modifyOutputForm").attr("action",getURLString("/project/template/modifyOutputAction"));
	var param = new Object();
	formSubmit("modifyOutputForm", param, "${e3ps:getMessage('산출물을 수정하시겠습니까?')}", function(){
		if(opener.window.viewTask) {
			opener.window.viewTask("${taskOid}");
		}
	}, true);
}
...
function deleteOutput(){
	...
	$("#modifyOutputForm").attr("action",getURLString("/project/template/deleteOutputAction"));
	var param = new Object();
	formSubmit("modifyOutputForm", param, "${e3ps:getMessage('삭제하시겠습니까?')}", function(){
		if(opener.window.viewTask) {
			opener.window.viewTask("${taskOid}");
		}
	}, true);
