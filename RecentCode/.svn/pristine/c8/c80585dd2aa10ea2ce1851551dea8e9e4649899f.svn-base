...
public class AfterEDRloader 
{
	 ...
	 private static boolean VERBOSE = true;//getVerboseProperty();

	 public static String[] copyToEPM(Representable repable,
                                      Persistable object,
                                      Representation rep,
	 ...
	 throws WTException {
		  ...
		  if (object instanceof EPMDocument) {
		 	  // the session is changed to administrator with SessionHelper.manager.setAdministrator(). The method runs under administrator during each invocation
			  // get current user
			  WTPrincipal realUser = SessionHelper.manager.getPrincipal();
			  if (VERBOSE) System.out.println("realUser..." + realUser);

			  // set session user to Administrator
			  SessionHelper.manager.setAdministrator();
			  if (VERBOSE) System.out.println("setAdministrator..." + SessionHelper.manager.getPrincipal());
			 
			  EPMDocument cadDoc = (EPMDocument)object;
			  String cadDivision = cadDoc.getAuthoringApplication().toString();
			  String cadType = cadDoc.getDocType().toString();
			  if (VERBOSE) {		 
...
//						  if (t.equals("zip")) t = tok.nextToken();
			  ...
			  if (VERBOSE) System.out.println(".copyToEPM() END");
			  //set back to current user
			  //SolidWorks Drawing publish File PDF,DWG 파일 도면으로 생성
    ...
    private static String copyVisualizations(EPMDocument epmDoc, Representation rep, String viewableExt, Boolean downloadToLocal) throws WTException {
...
//					File dir = new File(localDir + File.separator + "Design_deliverables" + File.separator + container + File.separator + authapp + File.separator + epmdocType + "_" + fm.format(Calendar.getInstance().getTime()));
			...
			for (int i = 0; i < appDatas.size(); i++) {
				...
				if (fileName != null && fileName.indexOf("." + viewableExt) != -1) {
					...
					if (!sFileName.equals(fileName)) {
						if (downloadToLocal) ret = downloadContent(epmDoc, appData, directory, fileName, sFileName);
	...
	private static ApplicationData copyApplicationData(ContentHolder ch, ApplicationData source, String sFileName) throws WTException, WTPropertyVetoException {
	...
	private static boolean deletecopyToEPMContents(ContentHolder ch, String viewableExt) throws WTException, PropertyVetoException {

		String sName = ((EPMDocument)ch).getCADName();
		ch = (EPMDocument)ContentHelper.service.getContents(ch);
		
		/* Get the list of ContentItem out of the passed ContentHolder This should be done after a call to ContentHelper.service.getContents( ContentHolder ) 
		Note this will NOT return the primary content for a FormatContentHolder. */
...
//		appDatas = ContentHelper.getContentListExcludeRoles(ch, "THUMBNAIL,THUMBNAIL3D,THUMBNAIL_SMALL,CAD_HIDDEN_CONTENT");
		...
		for (int i = 0; i < appDatas.size(); i++) {
			...
			if (fileName.indexOf(sName) != -1 && fileName.indexOf("."+viewableExt) != -1) {
				...
				if (VERBOSE) System.out.println("Removing..."+ fileName);
	...
	private static String getcopyToEPMFileName(ContentHolder ch, String appFileName, String viewableExt) throws WTException {
...
//		String versionInfo = ((EPMDocument)ch).getVersionDisplayIdentity().toString();
		...
		{
			sFileName = sFileName.substring(0, sFileName.indexOf(".")) + "_" + iterationInfo +  "." + viewableExt; // <CADName>_<iterationInfo>.extension (ex. ABCED-FGH-I_A.1.pdf)
...
//			sFileName = sFileName.substring(0, appFileName.indexOf(".")) + "_" + versionInfo +  "." + viewableExt; // <CADName>_<versionInfo>.extension (ex. ABCED-FGH-I_Rev A.pdf)
	...
	public static String downloadContent(EPMDocument epmDoc, ApplicationData appData, String dir, String fromFileName, String toFileName) {
		...
		try {
			...
			if (VERBOSE) System.out.println("downloadContent..." + ret);
...
/**
    private static String getDownloadToLocalDir() {
		String localDir = "";
		try {
			String tmp = WVSProperties.getPropertyValue("publish.afterloadermethod.copyToEPM.downloadToLocal.dir");
...
**/
    ...
    private static String getDownloadToLocalDir2() {
		...
		try {
			...
			if (VERBOSE) System.out.println("AFTERLOADERDIR: "+ tmp);
			if (tmp == null || tmp.length() == 0) {
				return localDir;
			}
			localDir = tmp;
                                     ...
                                     int structureType)
	{
		...
		if (object instanceof EPMDocument) {
			...
			try {
				...
				if (VERBOSE) System.out.println("renameAdditionalfiles() START");
				if (VERBOSE) System.out.println("renameAdditionalfiles : Param cadDoc : "+cadDoc );
				
//				if("PROE".equals(cadDoc.getAuthoringApplication().toString()) && "CADDRAWING".equals(cadDoc.getDocType().toString())) {
//					
//					WTPart refPart = null;
//					ApplicationData appData = null;
//					Vector<?> appDatas = null;
...
//					String fileExt = null;
//					InputStream is = null;
//					EPMDocument epm3D = EpmHelper.manager.get2DTo3DEPM(cadDoc.getPersistInfo().getObjectIdentifier().getStringValue());
//					if(epm3D != null) {
//						refPart = PartHelper.manager.getWTPart(epm3D);
//					}	
...
//						String fileRename = "";
//						
//						//번호 버전 이름 가져오는 우선순위, 버전의 경우 우선순위가 3D, 2D 순( 부품의 버전쓰지 않음 )
//						//1.WTPart 
//						//2.3D(EPMDocument) 
//						//3.2D(EPMDocument)
//						String number = "";
...
//						
//						if(refPart != null) {
//							number = refPart.getNumber();
//							version = epm3D.getVersionIdentifier().getValue() + epm3D.getIterationIdentifier().getValue();
//							name = refPart.getName();
//						}else if(epm3D != null) {
//							number = epm3D.getNumber();
//							version = epm3D.getVersionIdentifier().getValue() + epm3D.getIterationIdentifier().getValue();
//							name = epm3D.getName();
//						}else {
//							number = cadDoc.getNumber();
//							version = cadDoc.getVersionIdentifier().getValue() + cadDoc.getIterationIdentifier().getValue();
//							name = cadDoc.getName();
//						}
//						
...
//							
//							fileRename = number.toUpperCase() + "_" + version + "_" + name + "." + fileExt;						
//							
//							appData.setFileName(fileRename);
//							
//							is = ContentServerHelper.service.findContentStream(appData);
