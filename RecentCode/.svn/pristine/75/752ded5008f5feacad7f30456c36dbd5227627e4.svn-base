...
webix.ready(function(){
	var grid = webix.ui({
		...
		datafetch:50,
		columns:[
			{ id : "distributeNumber",		header : "${e3ps:getMessage('배포 번호')}",		width:"120",
			...
			{ id : "distributeName",		header : "${e3ps:getMessage('배포 제목')}",		width:"120", fillspace:true,
				css: "custom_webix_ellipsis custom_webix_center",
			},
			{ id : "supplierName",		header : "${e3ps:getMessage('배포 업체')}",		width:"120",
				css: "custom_webix_ellipsis custom_webix_center",
			},
			...
			{ id : "pjtNo",		header : "${e3ps:getMessage('프로젝트 번호')}",		width:"120",
				css: "custom_webix_ellipsis custom_webix_center",
			},
			{ id : "requestNumber",		header : "${e3ps:getMessage('발주 번호')}",		width:"120",
				css: "custom_webix_ellipsis custom_webix_center",
			},
			{ id : "type",		header : "${e3ps:getMessage('타입')}",		width:"100",
				css: "custom_webix_ellipsis custom_webix_center",
			},
			{ id : "info", header : "", width:"100",
				css: "custom_webix_ellipsis custom_webix_center",
				template:function(obj){
					return "<img class='pointer' src='/Windchill/netmarkets/images/details.gif' onclick='javascript:openItemInfo(\"" + obj.oid + "\",\"" + obj.item_oid + "\",\"" + obj.number + "\");'>";
				},
			},
			{ id : "number",		header : "${e3ps:getMessage('번호')}",		width:"120",
				css: "custom_webix_ellipsis",
				template:function(obj){
					var oid = obj.oid;
					return "<a href='#' onclick='openView(\""+oid+"\")'>"+obj.number+"</a>";
				},
			},
			{ id : "name",		header : "${e3ps:getMessage('이름')}",		width:"120",
				css: "custom_webix_ellipsis",
			},
...
function getGridData(){
	$$("grid_wrap").load(function(params){
		...
		return webix.ajax().headers({
		    ...
		    }).post(getURLString("/distribute/searchDistributeItemAction"), params)
...
<div class="product">
		...
		<form name="searchForm" id="searchForm">
			...
			<table class="mainTable">
				...
				<tbody>
					...
					<tr>
						<th>${e3ps:getMessage('타입')}</th>
						<td>
							<input type="radio" id="type" name="type" value="" data-rolecode="" checked>
							<label>${e3ps:getMessage('전체')}</label>
							<input type="radio" id="type" name="type" value="part">
							<label>${e3ps:getMessage('부품')}</label>
							<input type="radio" id="type" name="type" value="epm">
					...
					<tr>
						...
						<th scope="col">${e3ps:getMessage('프로젝트')}</th>
						<td>
							<div class="pro_view">
					...
					<tr>
						<th scope="col">${e3ps:getMessage('문서')}</th>
						<td>
							<div class="pro_view">
								<select class="searchRelatedObject" id="relatedDoc" name="relatedDoc" multiple data-param="doc"></select>
							</div>
						</td>
						<th scope="col">${e3ps:getMessage('도면')}</th>
						<td>
							<div class="pro_view">
								<select class="searchRelatedObject" id="relatedEpm" name="relatedEpm" multiple data-param="epm"></select>
							</div>
						</td>
					</tr>
					<tr>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownDistributeItem');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
