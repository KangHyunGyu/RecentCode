...
<script>
	...
	$(document).ready(function() {
		...
		//구매요청 구분  
		//let purchaseTypeList = getAUIErpCodeList('purchaseType');
		//setSelectBox('purchaseType', purchaseTypeList, true, false, false);  
		...
		cpc_companyAutoSearch('distributeCompany');
		
		cpc_companyUserAutoSearch('distributeTarget');
	...
	var columnLayout = [
			{
				...
				renderer : {
					...
					jsCallback : function(rowIndex, columnIndex, value, item) {
						var url = getURLString("/distribute/viewDistributeRegistration")
								+ "?oid=" + item.oid;
				...
				dataField : "distName",
				headerText : "${e3ps:getMessage('배포 요청 명')}",
				style : "AUIGrid_Left",
				width : "*",
				filter : {
					showIcon : true,
				...
				dataField : "distributeCompanyName",
				...
				dataField : "distributeTargetName",
				headerText : "${e3ps:getMessage('배포 유저')}",
				width : "8%",
				filter : {
					showIcon : true,
					iconWidth : 30,
	...
	function getGridData() {
		...
		$("#searchForm")
				.attr(
						"action",
						getURLString("/distribute/searchDistributeRegistrationScrollAction"));
  ...
  <!-- button -->
	<div class="seach_arm pt10 pb5">
		...
		<div class="rightbt">
			...
			<button type="button" class="s_bt03" id="searchBtn" onclick="switchDetailBtn();">${e3ps:getMessage('상세 검색')}</button>
		...
		<form name="searchForm" id="searchForm">
			...
			<table class="mainTable">
				...
				<tbody>
					...
					<tr>
						<th scope="col">${e3ps:getMessage('배포일')}</th>
						<td colspan="1">
							<input type="text" class="datePicker w30" name="pre_distDate" id="pre_distDate" readonly /> ~ <input type="text" class="datePicker w30" name="post_distDate" id="post_distDate" readonly />
						</select>
						</td>
						<th scope="col">${e3ps:getMessage('상태')}</th>
						<td><select class="multiSelect w10" id="state" name="state" multiple style="height: 20px;overflow-y: hidden;"></select></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('배포 업체')}</th>
						<td>
							<select id="distributeCompany" name="distributeCompany" data-width="60%" style="overflow-y: hidden;"></select>
							<span class="pointer verticalMiddle" onclick="javascript:deleteDistributeDoc('distributeCompany');"><img class="verticalMiddle" src='/Windchill/jsp/portal/images/delete_icon.png'></span>
						</td>
						<th scope="col">${e3ps:getMessage('배포 유저')}</th>
						<td>
							<select id="distributeTarget" name="distributeTarget" data-width="60%" style="overflow-y: hidden;"></select>
							<span class="pointer verticalMiddle" onclick="javascript:deleteDistributeDoc('distributeTarget');"><img class="verticalMiddle" src='/Windchill/jsp/portal/images/delete_icon.png'></span>
						</td>
					</tr>
					...
					<tr class="switchDetail calendar">
						...
						<td><span id="autoPartSearch"> <select class="searchRelatedObject" id="relatedPart" name="relatedPart" multiple data-param="part" data-width="60%"></select> <span class="pointer verticalMiddle" onclick="javascript:deleteUser('relatedPart');"><img class="verticalMiddle" src='/Windchill/jsp/portal/images/delete_icon.png'></span></td>
