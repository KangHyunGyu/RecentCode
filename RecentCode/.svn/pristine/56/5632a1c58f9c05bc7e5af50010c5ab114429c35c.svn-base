...
//AUIGrid 칼럼 설정
var columnLayout = [
	{ dataField : "request_number",		headerText : "${e3ps:getMessage('발주 번호')}",		width:"9%" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "part_number",		headerText : "${e3ps:getMessage('발주 부품 번호')}",		width:"10%",	style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "part_name",		headerText : "${e3ps:getMessage('발주 부품 명')}",		width:"12%", style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "part_version",		headerText : "${e3ps:getMessage('발주 부품 버전')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pr_user_id",		headerText : "${e3ps:getMessage('PR 요청자 사번')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pr_user_name",		headerText : "${e3ps:getMessage('PR 요청자 이름')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pr_user_email",		headerText : "${e3ps:getMessage('PR 요청자 e-mail')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "request_date",		headerText : "${e3ps:getMessage('납품 기일')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "if_cdateStr",		headerText : "${e3ps:getMessage('I/F 생성일')}",		width:"11%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "if_rdateStr",		headerText : "${e3ps:getMessage('I/F 수령일')}",		width:"11%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "if_statusStr",		headerText : "${e3ps:getMessage('I/F 상태')}",		width:"7%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchPurchaseListInfoScrollAction"));
...
<div class="product">
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<td><input type="text" id="request_number" name="request_number" class="w100"></td>
						<th scope="col">${e3ps:getMessage('발주 부품 번호')}</th>
						<td><input type="text" id="part_number" name="part_number" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('발주 부품 명')}</th>
						<td><input type="text" id="part_name" name="part_name" class="w100"></td>
						<th scope="col">${e3ps:getMessage('납품기일')}</th>
						<td class="calendar">
					        <input type="text" class="datePicker w25" name="request_predate" id="request_predate" readonly/>
							~
							<input type="text" class="datePicker w25" name="request_postdate" id="request_postdate" readonly/>
						</td>
					</tr>
					<tr>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPPurchaseList');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
