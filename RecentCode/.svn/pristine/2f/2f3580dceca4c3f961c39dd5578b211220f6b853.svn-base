...
$(document).ready(function(){
	...
	AUIGrid.destroy(app_line_myGridID);
...
var app_line_columnLayout = [
	{ dataField : "roleType",				headerText : "${e3ps:getMessage('구분')}",			width:"30%",	
		editRenderer : {
	...
	{ dataField : "departmentName",			headerText : "${e3ps:getMessage('부서')}",			width:"30%",	editable: false},
	{ dataField : "name",				headerText : "${e3ps:getMessage('이름')}",			width:"*",	style:"aui-grid-editable-cell",
	...
	/* { dataField : "duty",				headerText : "${e3ps:getMessage('직위')}",			width:"*%",	editable: false}, */
...
function app_line_forming(form){
	
	var param = form
	
	var approvalList = AUIGrid.getGridData(app_line_myGridID);
	
	Array.prototype.forEach.call(approvalList, function(appItem, idx){
		param.append("approval_line_roleType", appItem.roleType);
		param.append("approval_line_people", appItem.pOid);
	});
...
function app_validation(){
	...
	for(var i = 0 ; i < approvalList.length ; i++){
		console.log("roleType = " +approvalList[i].roleType);
	}
...
<div class="seach_arm2 pt15 pb5">
	...
	<div class="rightbt">
		...
		<button type="button" class="s_bt03" style="width:70px" onclick="addApprovalLinePopup()">${e3ps:getMessage('결재선')}</button>
	</div>
</div>
<!-- //button -->
<div class="list" id="app_line_grid_wrap" style="height:${gridHeight}px;margin-bottom:100px;">
</div>