...
//AUIGrid 칼럼 설정
var columnLayout = [
	{ dataField : "code",		headerText : "${e3ps:getMessage('코드')}",		width:"11%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "old_code",		headerText : "${e3ps:getMessage('OLD 코드')}",		width:"11%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "name",		headerText : "${e3ps:getMessage('코드 명칭')}",		width:"11%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchSiteInfoScrollAction"));
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<th scope="col">${e3ps:getMessage('OLD 코드')}</th>
						<td><input type="text" id="old_code" name="old_code" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('코드 명')}</th>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<c:if test="${e3ps:isSyncBtn()}">
				<button type="button" class="s_bt03" onclick="sync('syncERPSite');">${e3ps:getMessage('동기화')}</button>
			</c:if>
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPSite');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
