...
import com.ptc.windchill.uwgm.common.util.CadDocCommonUtilities;
...
//	}
	...
	public void deleteDistributeAction(Map<String, Object> reqMap) throws Exception {
		...
		try {
			...
			if(oid.length() > 0) {
				...
				ApprovalHelper.service.deleteApproval(distribute);
	...
	@Override
	public void deleteDistributeRegistrationAction(Map<String, Object> reqMap) throws Exception {
...
//			}
	...
	@Override
	public void modifyDistributeRegistrationAction(Map<String, Object> reqMap) throws Exception {
		Transaction trx = new Transaction();
		...
		try {
			...
			List<Map<String,Object>> distributeParts = (List<Map<String,Object>>)reqMap.get("distributeParts");
			...
			distributeReg = (DistributeRegistration) CommonUtil.getObject(oid);
			...
			distributeReg = (DistributeRegistration) PersistenceHelper.manager.modify(distributeReg);
			
			//배포요청 부품 리스트 삭제
			deleteDistributePartLink(distributeReg);
			
			//배포요청 부품 재생성
			for(Map<String,Object> map : distributeParts) {
				createDistributeRegPartLink(map, distributeReg);
			}
			
			//attach files
			CommonContentHelper.service.isDataExcludedDelete((ContentHolder)distributeReg, ContentRoleType.SECONDARY, delocIds);
			if(secondary.size() > 0) {
				for (int i = 0; i < secondary.size(); i++) {
					...
					CommonContentHelper.service.attach((ContentHolder)distributeReg, cacheDs, fileName, null, ContentRoleType.SECONDARY);
			    }
			}
	...
	@Override
	public void deleteDistributePartLink(DistributeRegistration distributeReg) throws Exception {
		
		WTSet deleteList = new WTHashSet();
		
		List<DistributeRegToPartLink> partLinkList = DistributeHelper.manager.getDistributePartListByDistribute(distributeReg);
		
		for(DistributeRegToPartLink link : partLinkList) {
			//배포 링크 삭제
			deleteList = deleteDistributePartToEpmLink(link, deleteList);
		}
		
		deleteList.addAll(partLinkList);
		
		PersistenceHelper.manager.delete(deleteList);
...
//	}
	...
	public WTSet deleteDistributePartToEpmLink(DistributeRegToPartLink distPartLink, WTSet deleteList) throws Exception {
    	QueryResult results = PersistenceHelper.manager.navigate(distPartLink, DistributeRegPartToEpmLink.DISTRIBUTE_REG_PART_TO_EPM_ROLE, DistributeRegPartToEpmLink.class, false);
    	deleteList.addAll(results);
        return deleteList;
	}
	...
	public List<Map<String, Object>> createRegistrationDistributeAction(Map<String, Object> reqMap) throws Exception {
		...
		try {
			...
			//attach files
			CommonContentHelper.service.attach((ContentHolder)distributeReg, null, secondary);
