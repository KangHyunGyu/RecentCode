...
function addECR() {
	var selectedECR = AUIGrid.getCheckedRowItems(myGridID);
	
	if(selectedECR == ''){
		openNotice("${e3ps:getMessage('ECR을 선택하세요.')}");
		return false;
	}
	
	selectedECR.forEach(function(ecr) {
		
		if(ecr.item.state != '승인됨') {
			alert("승인된 ECR만 추가 가능합니다.");
			return;
		}
	    
		console.log(ecr.item);
		
	    var optionHtml = "<option value='" + ecr.item.oid + "'>" + ecr.item.name + "</option>";
	    $("#relatedECR", opener.document).append(optionHtml);
	    
	    if(opener.window.setRelatedECR && typeof opener.window.setRelatedECR == 'function') {
			if(!confirm("${e3ps:getMessage('선택하신 ECR을 추가하시겠습니까?')}")){
				return false;
			}
			opener.window.setRelatedECR(ecr.item);
		}
	...
	<div class="pl30 pr30">
		<div class="seach_arm2 pt10 pb5">
			...
			<button type="button" class="s_bt05" id="resetBtn" onclick="reset();">${e3ps:getMessage('초기화')}</button>
				<button type="button" class="s_bt03" onclick="javascript:addECR()">${e3ps:getMessage('추가')}</button>
				...
				<table class="mainTable">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="pro_view">
									<select class="searchUser" id="changeOwner" name="changeOwner" multiple data-width="60%">
									</select>
