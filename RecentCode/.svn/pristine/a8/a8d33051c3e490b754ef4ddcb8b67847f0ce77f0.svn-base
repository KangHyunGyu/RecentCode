...
var erpTree_columnLayout = [
	...
	{ dataField : "version",			headerText : "${e3ps:getMessage('버전')}",				width:"20%",
	},
...
function erpTree_getGridData(){
	...
	param.pjtNo = $("#pjtNo").val();	
	param.partNo = $("#partNo").val();
	
	$("#openLevel").val(1);
	
	AUIGrid.showAjaxLoader(erpTree_myGridID);
	var url = getURLString("/erp/getERPBom");
	ajaxCallServer(url, param, function(data){
		...
		// 그리드에 데이터 세팅
		AUIGrid.setGridData(erpTree_myGridID, list);
...
function erpTree_search() {
	...
	AUIGrid.search(erpTree_myGridID, "number", searchERPPartNo, options);
...
<div class="pop">
	...
	<div class="top">
		...
		<c:if test="${pjtNo ne ''}">
			<h2>ERP BOM / ${pjtNo}</h2>
			<input type="hidden" id="pjtNo" name="pjtNo" value="${pjtNo}">
		</c:if>
	...
	<div class="con pl25 pr25 pb15">
		<div class="seach_arm2 pt10 pb5">
			<div class="leftbt">
				<c:if test="${pjtNo eq ''}">
					<span>${e3ps:getMessage('프로젝트 번호')} : </span>
					<input type="text" id="pjtNo" name="pjtNo" class="w20">
				</c:if>
				<c:if test="${pjtNo ne ''}">
					<input type="hidden" id="pjtNo" name="pjtNo" value="${pjtNo}">
				</c:if>
