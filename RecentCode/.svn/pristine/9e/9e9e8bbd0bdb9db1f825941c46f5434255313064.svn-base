...
function auiGridCompleteHandler(){
	// 수평 스크롤 이동 시킴 칼럼 인덱스
	AUIGrid.setHScrollPosition(myGridID, 12); 
}

function getGridData(){

	$("#searchForm").attr("action", getURLString("/erp/searchProjectUserInfoScrollAction"));
...
<div class="product">
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<th scope="col">${e3ps:getMessage('역할')}</th>
						<td><input type="text" id="role" name="role" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('구성원 사번')}</th>
						<td><input type="text" id="user_id" name="user_id" class="w100"></td>
						<th scope="col">${e3ps:getMessage('구성원 이름')}</th>
						<td><input type="text" id="user_name" name="user_name" class="w100"></td>
					</tr>
					<tr>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPProjectUser');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
