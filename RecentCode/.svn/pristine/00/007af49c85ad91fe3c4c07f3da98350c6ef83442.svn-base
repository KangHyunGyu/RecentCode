...
//AUIGrid 칼럼 설정
var columnLayout = [
	{ dataField : "part_number",		headerText : "${e3ps:getMessage('부품 번호')}",		width:"8%",	style:"AUIGrid_Left", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "part_name",		headerText : "${e3ps:getMessage('부품 명')}",		width:"14%",		style:"AUIGrid_Left", 
		filter : {
			showIcon : true,
			iconWidth:30
		}		
	},
	{ dataField : "part_version",		headerText : "${e3ps:getMessage('버전')}",		width:"6%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "modify_date",		headerText : "${e3ps:getMessage('등록/수정일자')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "classification",		headerText : "${e3ps:getMessage('부품 분류')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "part_type",		headerText : "${e3ps:getMessage('부품대속성')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "unit",		headerText : "${e3ps:getMessage('단위 코드')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "snok",		headerText : "${e3ps:getMessage('S/N관리 여부')}",		width:"13%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "purchaseflag",		headerText : "${e3ps:getMessage('구매여부')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "description",		headerText : "${e3ps:getMessage('부품 명(CAD)')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "specification",		headerText : "${e3ps:getMessage('규격')}",		width:"6%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "material",		headerText : "${e3ps:getMessage('재질')}",		width:"6%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "summary",		headerText : "${e3ps:getMessage('개요')}",		width:"6%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "notice",		headerText : "${e3ps:getMessage('비고')}",		width:"9%",
		filter : {
			showIcon : true,
			iconWidth:30
		}		
	},
	{ dataField : "cpartnum",		headerText : "${e3ps:getMessage('참조 부품 번호')}",		width:"13%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "endDate",		headerText : "${e3ps:getMessage('종료일자')}",		width:"13%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "drw_url",		headerText : "${e3ps:getMessage('도면 URL')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchPartInfoScrollAction"));
...
<div class="product">
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<th scope="col">${e3ps:getMessage('부품 제목')}</th>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPPart');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
