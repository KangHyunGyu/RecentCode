...
public class IBAUtil {
     ...
     */
    public static DefaultAttributeContainer getAttrContainer(Hashtable attrs, Hashtable values) throws IBAConstraintException, WTPropertyVetoException {
     ...
     * @throws IBAConstraintException
     */
    private static void setContainerValue(DefaultAttributeContainer container, AbstractValueView valueView, Object valObj) throws WTPropertyVetoException, IBAConstraintException {
    ...
    public static boolean getBooleanValue(IBAHolder ibaHolder, String attrName) throws RemoteException, WTException {
        ...
        select.appendAnd();
        select.appendSelect(new ClassAttribute(BooleanValue.class, BooleanValue.VALUE), new int[] { idx_StrVal }, false);

        SearchCondition where = new SearchCondition(new ClassAttribute(BooleanValue.class, "theIBAHolderReference.key.id"), "=", new ClassAttribute(
                target, "thePersistInfo.theObjectIdentifier.id"));
        select.appendWhere(where, new int[] { idx_StrVal, idx });
        select.appendAnd();
        where = new SearchCondition(new ClassAttribute(BooleanValue.class, "definitionReference.hierarchyID"), "=", new ClassAttribute(
        		BooleanDefinition.class, "hierarchyID"));
        select.appendWhere(where, new int[] { idx_StrVal, idx_StrDef });
        select.appendAnd();
        select.appendWhere(new SearchCondition(BooleanDefinition.class, "name", "=", attrName), new int[] { idx_StrDef });
    ...
    public static HashMap getAttributesHidNViewObject(IBAHolder ibaHolder) throws RemoteException, WTException, NumberFormatException, WTPropertyVetoException {
        IBAHolder iba = IBAValueHelper.service.refreshAttributeContainer(ibaHolder, null, null, null);
        DefaultAttributeContainer container = (DefaultAttributeContainer) iba.getAttributeContainer();
        HashMap hash = new HashMap();
        if (container == null) return hash;
    ...
    public static Object getIBAObject(IBAHolder ibaHolder, String varName, String ibaType){
    	...
    	try{
			...
			qs.appendAnd();
			qs.appendWhere(new SearchCondition(cls,"theIBAHolderReference.key.id",SearchCondition.EQUAL,longOid),new int[] { idx });
			qs.appendAnd();
    ...
    public static void changeIBAValue(IBAHolder ibaHolder, String varName, String value) throws NumberFormatException, RemoteException, WTPropertyVetoException, WTException {
        if (value == null) { return; }
        else if (value.length() == 0) {
        ...
        if (!my.containsKey(hid)) {
            ...
            else if (aview instanceof BooleanDefView) createIba(ibaHolder, "boolean", varName, value);
        } else {
     ...
     * @throws WTException
     */
    public static void deleteIBA(IBAHolder ibaHolder, String varName) throws NumberFormatException, RemoteException, WTPropertyVetoException, WTException {
...
//        // ((InstanceAttributeIdentifier) searchExp.getIdentifier()) .getAttributeDefinition();
...
//    }
    ...
    public static StringDefinition getStringDefinition(String attrName) throws WTException {
        QuerySpec select = new QuerySpec();
                ...
                if (nodeName.equalsIgnoreCase(orgName)) {
                    ...
                    for (int j = 0; j < defNodeView.length; j++) {
                        ...
                        if (valueView instanceof UnitValueDefaultView) {
                            hash.put(hid, new AttributeData(aview.getLocalizedDisplayString(), aview, ((UnitValueDefaultView) valueView).toUnit().getUnits()));
                        	...
                        	hash.put(hid, new AttributeData(aview.getName(), aview.getLocalizedDisplayString(), aview, "boolean"));
                        }
                    }
                    break;
                }
            ...
            for (int i = orgNodeView.length - 1; i > -1; i--) {
                ...
                if (nodeName.equalsIgnoreCase(orgName)) {
                    ...
                    for (int j = 0; j < defNodeView.length; j++) {
                        aview = getDefaultViewObject((AttributeDefNodeView) defNodeView[j]);
                        valueView = (AbstractValueView) NewValueCreator.createNewValueObject(aview);
                        if (valueView instanceof UnitValueDefaultView) hash.put(aview.getName(), new AttributeData(aview.getName(), aview.getLocalizedDisplayString(), aview, ((UnitValueDefaultView) valueView).toUnit().getUnits()));
                        else if (valueView instanceof StringValueDefaultView) hash.put(aview.getName(), new AttributeData(aview.getName(), aview.getLocalizedDisplayString(), aview, "string"));
                        else if (valueView instanceof FloatValueDefaultView) hash.put(aview.getName(), new AttributeData(aview.getName(), aview.getLocalizedDisplayString(), aview, "float"));
                        else if (valueView instanceof TimestampValueDefaultView)hash.put(aview.getName(), new AttributeData(aview.getName(), aview.getLocalizedDisplayString(), aview, "timestamp"));
                        else if (valueView instanceof BooleanValueDefaultView)hash.put(aview.getName(), new AttributeData(aview.getName(), aview.getLocalizedDisplayString(), aview, "boolean"));
                    }
                    break;
                }
            }
            ...
            while (eu.hasMoreElements()) {
                ...
                if (!flag) {
                    ...
                    if (dataType.equals("string")) {
                    	...
                    	BooleanValueDefaultView view = (BooleanValueDefaultView) NewValueCreator.createNewValueObject(aview);
                    	view.setValue(Boolean.parseBoolean(data.value));
                    	container.addAttributeValue(view);
                    } else {
...
//            else if (mode.equals("boolean")) idx_IbaValue = query.addClassList(wt.iba.value.BooleanValue.class, false);
...
//                }
//                if (query.getConditionCount() > 0) query.appendAnd();
//                SearchCondition sc = new SearchCondition(new ClassAttribute(wt.iba.value.BooleanValue.class, "theIBAHolderReference.key.id"), "=",
//                        new ClassAttribute(target, "thePersistInfo.theObjectIdentifier.id"));
//                query.appendWhere(sc, new int[] { idx_IbaValue, idx });
//                query.appendAnd();
//                setupQuerySpecForInstance(query, range.getSearchExpression(aview, null), idx_IbaValue);
...
//    }
    ...
    /**
     * 멀티 검색
     * @methodName : appendIBAStringListWhere
     * @author : tsuam
     * @date : 2021.10.21
     * @return : void
     * @description :
     */
    public static void appendIBAStringListWhere(QuerySpec query, Class target, int idx, String ibaName, List<String> paramsList ) throws Exception{
		...
		if (aview != null) {
			...
			query.appendAnd();
			query.appendWhere(new SearchCondition(new ClassAttribute(StringValue.class, StringValue.VALUE2),
					SearchCondition.IN, new ArrayExpression(paramsList.toArray())),
					new int[] { _idx });
		}
    ...
    /**
     * Value 에서 값이 n개 aaa,bbb,ccc
     * @methodName : appendIBAStringValueListWhere
     * @author : tsuam
     * @date : 2021.12.13
     * @return : void
     * @description :
     */
    public static void appendIBAStringValueListWhere(QuerySpec query, Class target, int idx, String ibaName, List<String> paramsList ) throws Exception{
		...
		if (aview != null) {
			...
			int cnt = 1;
			for(String value : paramsList) {
				
				query.appendWhere(new SearchCondition(StringValue.class, StringValue.VALUE,SearchCondition.LIKE, "%"+value.toUpperCase()+"%"),
						new int[] { _idx });
				if(cnt != paramsList.size()) {
					query.appendOr();
				}
				cnt++;
			}
    ...
    /**
     * StringValue 검색
     * @methodName : appendIBAStringEqual
     * @author : yhjang
     * @date : 2021.11.17
     * @return : void
     * @description :
     */
    public static void appendIBAStringEqual(QuerySpec query, Class target, int idx, AttributeDefDefaultView aview, String value) throws Exception{
    	
    	appendIBAString( query,  target,  idx,  aview,  value, SearchCondition.EQUAL);
    }
    
    /**
     * StringValue 검색
     * @methodName : appendIBAStringNotEqual
     * @author : yhjang
     * @date : 2021.11.17
     * @return : void
     * @description :
     */
    public static void appendIBAStringNotEqual(QuerySpec query, Class target, int idx, AttributeDefDefaultView aview, String value) throws Exception{
    	
    	appendIBAString( query,  target,  idx,  aview,  value, SearchCondition.NOT_EQUAL);
    }
    
    /**
     * StringValue 검색
     * @methodName : appendIBAString
     * @author : yhjang
     * @date : 2021.11.17
     * @return : void
     * @description :
     */
    public static void appendIBAString(QuerySpec query, Class target, int idx, AttributeDefDefaultView aview, String value , String SearchType) throws Exception{
    	
    	if (!WTStringUtilities.isEmpty(value)) {
    		
    		if (query.getConditionCount() > 0) {
    			query.appendAnd();
    		}
    	
			int idxIbaVal = query.appendClassList(StringValue.class, false);
			
			query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",SearchCondition.EQUAL,aview.getHierarchyID()),
					new int[] { idxIbaVal });
			
			query.appendAnd();
			
			query.appendWhere(
					new SearchCondition(new ClassAttribute(StringValue.class, "theIBAHolderReference.key.id"), "=",
							new ClassAttribute(target, "thePersistInfo.theObjectIdentifier.id")),
					new int[] { idxIbaVal, idx });
			
			query.appendAnd();
	
			query.appendWhere(new SearchCondition(StringValue.class, StringValue.VALUE,SearchType,value.toUpperCase()),
					new int[] { idxIbaVal });
    	}
	}
    
    public static StringValue getStringAttr(IBAHolder ibaHolder, String attrName) throws RemoteException, WTException {
    	StringValue returnStr = null;
        if (ibaHolder == null) {
        	return returnStr;
		}
        Class target = ibaHolder.getClass();
        ...
        int idx = select.addClassList(target, false);
        int idx_StrVal = select.addClassList(StringValue.class, true);
        int idx_StrDef = select.addClassList(StringDefinition.class, false);
        long lon = ((Persistable)ibaHolder).getPersistInfo().getObjectIdentifier().getId();
...
/*******************************************************************************
 * $Log: IBAUtil.java,v $
 * Revision 1.1  2016/11/07 01:48:58  e3ps
 * *** empty log message ***
 *
 * Revision 1.4  2016/02/16 02:43:13  repair
 * *** empty log message ***
 *
 * Revision 1.1  2016/01/26 02:19:36  administrator
 * *** empty log message ***
 *
 * Revision 1.3  2015/10/19 02:00:41  hmkim
 * *** empty log message ***
 *
 * Revision 1.2  2015/09/17 04:54:29  sjlee
 * 2015-09-17 �ʱ� CVS ����
 *
 * Revision 1.1  2015/08/31 01:56:22  sjlee
 * *** empty log message ***
 *
 * Revision 1.2  2015/08/07 06:57:25  hmkim
 * *** empty log message ***
 *
 * Revision 1.1  2015/07/16 09:30:36  hmkim
 * *** empty log message ***
 *
 * Revision 1.1  2014/01/09 07:04:34  wslee
 * 2014-01-09 �������� CVS ����
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.1  2013/12/20 04:46:52  wslee
 * 20131220 �������� ���
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.4  2013/09/09 09:02:01  shkim
 * *** empty log message ***
 *
 * Revision 1.3  2013/08/08 05:34:57  tsuam
 * *** empty log message ***
 *
 * Revision 1.2  2013/08/08 05:08:43  tsuam
 * *** empty log message ***
 *
 * Revision 1.1  2013/07/09 08:44:04  hmkim
 * �߰� commit
 *
 * Revision 1.6  2013/04/30 04:26:18  wslee
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.5  2013/04/18 02:18:58  wslee
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.4  2013/04/17 08:25:18  cklee
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.3  2013/04/16 10:26:37  cklee
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.2  2013/03/07 07:02:37  wslee
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.1  2012/04/17 06:07:08  ysjung
 * ���ʵ��
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.7  2011/10/25 09:32:22  sdhwang
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.6  2011/10/25 09:05:35  yhjang
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.5  2011/10/25 08:57:51  yhjang
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.4  2011/10/25 08:56:38  sdhwang
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.3  2011/10/25 08:41:06  yhjang
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 ...
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.1  2011/04/21 03:51:03  thhwang
 * *** empty log message ***
 *
 * Revision 1.1  2011/03/08 01:50:11  thhwang
 * 등록
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
