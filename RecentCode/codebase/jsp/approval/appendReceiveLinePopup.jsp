...
function appendReceiveLineAction(){
	
	openConfirm("수신인을 추가 적용하시겠습니까?", function(){
		var list = new Array();
		
		$(".approvalLineGrid").each(function(){
			const appLineList = $$($(this).attr("id")).serialize(true);
			
			for(var i=0; i < appLineList.length; i++){
				appLineList[i]["roleType"] = $(this).data("roletype");
				
				if(appLineList[i].isExReceiveLine == null || !appLineList[i].isExReceiveLine){
		...
		//param["type"] ="${type}";
		//
		//formSubmit("approveForm", param, $(btn).html() + "${e3ps:getMessage('하시겠습니까?')}");
		
		//getURLString("/approval/createTemplateAction");
		
		let param = new Object();
		param["appLineOid"] = "${param.oid}";
		param["appendReceiveList"] = list;
		
		console.log(list);
		ajaxCallServer(getURLString("/approval/appendReceiveLineAction"), param, function(){
...
function applyAppLine(){
	openConfirm("기존 결재선을 삭제하고 현재 결재선을 추가하시겠습니까?", function(){
		var list = new Array();
		...
		var listStringData = JSON.stringify(list);
		
		if (opener.window.app_line_getAppLineFromPopup) {
			opener.window.app_line_getAppLineFromPopup(listStringData);
...
<!-- pop -->
<div class="pop">
<form id="addApprovalLineForm" name="addApprovalLineForm" onsubmit="return false;">
	<!-- top -->
	<div class="top">
	...
	<div class="tap pt20">
		...
		<div class="tapbutton">
		<button type="button" class="s_bt03" onclick="appendReceiveLineAction();">${e3ps:getMessage('수신인 추가 적용')}</button>
			<%-- <button type="button" class="s_bt03" onclick="applyAppLine();">${e3ps:getMessage('결재선 적용')}</button>
			<button type="button" class="s_bt03" onclick="openTitleWindow();">${e3ps:getMessage('Template 저장')}</button> --%>
		</div>
	</div>
	<!--//tap -->
	...
	<div class="pop_semi">
		...
		<div class="pop_semitable3 mt20 ml10">
			...
			<jsp:include page="${e3ps:getIncludeURLString('/approval/include_approvalLineGridForAppend')}" flush="true">
				<jsp:param name="title" value="${e3ps:getMessage('수신')}"/>
				<jsp:param name="roleType" value="RECEIVE"/>
				<jsp:param name="appLineOid" value="${param.oid}"/>
			</jsp:include>
		</div>
	</div>
</form>
