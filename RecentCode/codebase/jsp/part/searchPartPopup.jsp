...
$(document).ready(function(){
	...
	//LifeCycle (RETURN,REWORK,CANCELLED) 제외
...
var columnLayout = [
	...
	{ dataField : "version",		headerText : "${e3ps:getMessage('버전')}",			width:"8.5%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	...
	{ dataField : "createDateFormat",		headerText : "${e3ps:getMessage('등록일')}",			width:"8.5%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "modifyDateFormat",		headerText : "${e3ps:getMessage('수정일')}",			width:"8.5%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "epm3DNumber",		headerText : "${e3ps:getMessage('3D')}",			width:"7%", style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	...
	{ dataField : "epm2DNumber",		headerText : "${e3ps:getMessage('2D')}",			width:"7%", style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	...
	{ dataField : "unit",		headerText : "${e3ps:getMessage('단위')}",			width:"4%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
	...
	{ dataField : "material",		headerText : "${e3ps:getMessage('소재')}",			width:"6%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
	...
	{ dataField : "surface",		headerText : "${e3ps:getMessage('표면처리')}",			width:"6%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
	...
	{ dataField : "spec",		headerText : "${e3ps:getMessage('규격')}",			width:"6%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
	...
	/*
	{ dataField : "partType",		headerText : "${e3ps:getMessage('총무게')}",			width:"6%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
	...
	{ dataField : "partType",		headerText : "${e3ps:getMessage('무게 단위')}",			width:"6%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
	...
	*/
	{ dataField : "partType",		headerText : "${e3ps:getMessage('비고')}",			width:"6%",	
		filter : {
			showIcon : true,
			iconWidth:30
		},	
...
function addPart(){
	...
	for(var i = 0; i < checkItemList.length; i++){
		checkItemList[i].item.targetRowId = "${rowId}";
		console.log(checkItemList[i].item.lastCheck);
		
		if("${moduleType}" == "ECO" || "${moduleType}" == "ECR"){
			if(checkItemList[i].item.state == "APPROVED") {
				if(checkItemList[i].item.lastCheck == true){
					addItemList.push(checkItemList[i].item);
				} else {
					alert("최신 버전 품목만 추가 가능합니다.");
					return;
				}	
			} else {
				alert("승인이 완료된 품목만 추가 가능합니다.");
				return;
			} 
		} else {
			addItemList.push(checkItemList[i].item);
	...
	} else if(opener.window.setPart){
		opener.window.setPart(addItemList);
	}
			...
			<div class="pro_table">
				...
				<tbody>
					...
					<tr>
						...
						<td >
							<select class="searchRelatedObject" id="numberMany" name="numberMany" multiple data-param="part" data-width="70%"></select>
							<span class="pointer verticalMiddle" onclick="javascript:deleteUser('numberMany');"><img class="verticalMiddle" src='/Windchill/jsp/portal/images/delete_icon.png'></span>
						</td>
						<th scope="col">${e3ps:getMessage('품목명')}</th>
						<td ><input type="text" id="nme" name="name" class="w78"></td>
					</tr>
					<tr>
						...
						<td>
							<div class="pro_view">
								<select class="searchUser" id="creatorFullName" name="creator" data-width="70%" multiple ></select>
								...
								<span class="pointer verticalMiddle" onclick="javascript:deleteUser('creatorFullName');"><img class="verticalMiddle" src='/Windchill/jsp/portal/images/delete_icon.png'></span>
							</div>
						</td>
					</tr>
					
					<tr class="switchDetail">
						<th scope="col">${e3ps:getMessage('자재유형')}</th>
						<td >
							<select class="multiSelect w30" id=materialType name="materialType" multiple style="height:20px;overflow-y: hidden;">
							</select>
						</td>
						<th scope="col">${e3ps:getMessage('제품군')}</th>
						<td >
							<select class="multiSelect w30" id="division" name="division" multiple style="height:20px;overflow-y: hidden;">
							</select>
						</td>
					</tr>
					<tr class="switchDetail">
					...
					<tr class="switchDetail">
						...
						<td>
							<div class="pro_view">
								<select class="searchUser" id="creator" name="creator" multiple data-width="62%">
								</select>
								<span class="pointer verticalMiddle" onclick="javascript:openUserPopup('creator', 'multi');"><img class="verticalMiddle" src="/Windchill/jsp/portal/images/search_icon2.png"></span>
...
<%-- 						<th scope="col">${e3ps:getMessage('품목 유형')}</th>	 --%>
<!-- 						<td> -->
<!-- 							<select class="multiSelect w10" id="viewName" name="viewName" multiple style="height:20px;overflow-y: hidden;"> -->
...
<!-- 						</td> -->
						...
						<td colspan="3">
							<div class="pro_view">
								<select class="searchUser" id="modifier" name="modifier" multiple data-width="62%">
								</select>
