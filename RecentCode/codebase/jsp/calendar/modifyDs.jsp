...
function save(){
	...
	$("#dsForm").attr("action",getURLString("/calendar/modifyDsAction"));
	var param = new Object();
	param["dsList"] = dsList;
	
	var msg = "${e3ps:getMessage('수정하시겠습니까?')}";
...
<form name="dsForm" id="dsForm" method="post">
	...
	<div class="seach_arm pt20 pb5">
		...
		<div class="rightbt">
			...
			<button type="button" class="s_bt03" onclick="save()">${e3ps:getMessage('수정')}</button>
			<button type="button" class="s_bt05" onclick="cancle()">${e3ps:getMessage('취소')}</button>
		</div>
	</div>
	<!-- //button -->
		...
		<table class="mainTable">
			...
			<tbody>
				<tr>
					...
					<td>
						<input type="text" class="w50" style="background-color: rgb(225, 225, 225);" id="name" name="name" value="${data.name}" readonly>
					</td>
				</tr>
				<tr>
					<th scope="col">${e3ps:getMessage('시작일 ~ 종료일')}<span class="required">*</span></th>
					<td class="calendar">
				        <input type="text" class="datePicker w25" name="startDate" id="startDate" value="${data.startDate}" readonly/>
						~
						<input type="text" class="datePicker w25" name="endDate" id="endDate" value="${data.endDate}" readonly/>
					</td>
				</tr>
				<tr>
					...
					<td class="pd15">
						<div class="textarea_autoSize">
							<textarea name="remark" id="remark" onkeydown="textAreaResize(this)" onkeyup="textAreaResize(this)">${data.remark}</textarea>
						</div>
					</td>
				</tr>
<!-- 	            <tr>  -->
...
<!-- 	            </tr> -->
	...
	<div class="ml30 mr30">
		<jsp:include page="${e3ps:getIncludeURLString('/calendar/include_addObject')}" flush="true">
			...
			<jsp:param name="fType" value="modify"/>
