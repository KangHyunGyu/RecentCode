	...
	<div class="pro_table mr30 ml30">
		...
		<input type="hidden" name="isManager"/>
			<table class="mainTable">
				...
				<tbody>
					<tr>
						<th scope="col">${e3ps:getMessage('템플릿 번호')}</th>
						<td><input type="text" id="number" name="number" class="w100"></td>
						<th scope="col">${e3ps:getMessage('템플릿 명')}</th>
...
var columnLayout = [
  {
    dataField: "code",
    headerText: "${e3ps:getMessage('템플릿 번호')}",
    width: "*%",
    sortValue: "code",
    style: "AUIGrid_Left",
    filter: {
  ...
  {
    dataField: "name",
    headerText: "${e3ps:getMessage('템플릿명')}",
    width: "30%",
    filter: {
      showIcon: true,
      iconWidth: 30,
  ...
  },
  {
    ...
    headerText: "${e3ps:getMessage('최초등록일')}",
    width: "10%",
    filter: {
      showIcon: true,
      iconWidth: 30,
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/project/template/searchTemplateAction"));
	param["type"] = "${type}";
...
function viewTemplate(oid){
 	var url = getURLString("/project/template/viewMain");
	url += "?oid=" + oid;
	location.href = url;
}
