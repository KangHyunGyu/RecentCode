...
function checkValidate() {
	...
	if(multiObjectList.length == 0) {
		openNotice("${e3ps:getMessage('일괄 결재 도면을 추가하세요.')}");
		return false;
	}
	...
	if(!multiObjectValid) {
		...
		openNotice("${e3ps:getMessage('추가된 일괄 결재 도면에 값을 입력하세요.')}");
		return false;
	}
...
<input type="hidden" id="objectType" name="objectType" value="${objType}">
	<!-- button -->
