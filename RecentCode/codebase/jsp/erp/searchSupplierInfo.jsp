...
//AUIGrid 칼럼 설정
var columnLayout = [
	{ dataField : "code",		headerText : "${e3ps:getMessage('업체 코드')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "id",		headerText : "${e3ps:getMessage('업체 유저 아이디')}",		width:"12%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "name",		headerText : "${e3ps:getMessage('업체 명')}",		width:"20%", style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "email",		headerText : "${e3ps:getMessage('대표 이메일')}",		width:"11%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "president",		headerText : "${e3ps:getMessage('대표자')}",		width:"7%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "business_condition",		headerText : "${e3ps:getMessage('업태')}",		width:"12%", style:"AUIGrid_Left" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "business_type",		headerText : "${e3ps:getMessage('업종')}",		width:"17%",  style:"AUIGrid_Left" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "tel_no",		headerText : "${e3ps:getMessage('전화 번호')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "fax_no",		headerText : "${e3ps:getMessage('팩스 번호')}",		width:"10%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "address",		headerText : "${e3ps:getMessage('주소')}",		width:"30%", style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "national_code",		headerText : "${e3ps:getMessage('국가 코드')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "enabled",		headerText : "${e3ps:getMessage('활성화 여부')}",		width:"11%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function auiGridCompleteHandler(){
	// 수평 스크롤 이동 시킴 칼럼 인덱스
	AUIGrid.setHScrollPosition(myGridID, 10); 
}
function getGridData(){

	$("#searchForm").attr("action", getURLString("/erp/searchSupplierInfoScrollAction"));
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<th scope="col">${e3ps:getMessage('업체 유저 아이디')}</th>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<c:if test="${e3ps:isSyncBtn()}">
				<button type="button" class="s_bt03" onclick="sync('syncERPSupplier');">${e3ps:getMessage('동기화')}</button>
			</c:if>
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPSupplier');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
