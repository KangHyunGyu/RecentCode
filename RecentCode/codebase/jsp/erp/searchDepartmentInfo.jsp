...
//AUIGrid 칼럼 설정
var columnLayout = [
	{ dataField : "p_code",		headerText : "${e3ps:getMessage('상위부서 코드')}",		width:"10%" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},		
	{ dataField : "code",		headerText : "${e3ps:getMessage('부서 코드')}",		width:"9%" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},		
	{ dataField : "ehr_p_code",		headerText : "${e3ps:getMessage('EHR P CODE')}",		width:"9%" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},		
	{ dataField : "ehr_code",		headerText : "${e3ps:getMessage('EHR CODE')}",		width:"9%" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},		
	{ dataField : "name",		headerText : "${e3ps:getMessage('부서 명')}",		width:"*", style:"AUIGrid_Left",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "enabled",		headerText : "${e3ps:getMessage('활성화 여부')}",		width:"9%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchDepartmentInfoScrollAction"));
		...
		<form name="searchForm" id="searchForm">
			<table class="mainTable">
				...
				<tbody>
					...
					<tr>
						<th scope="col">${e3ps:getMessage('EHR P CODE')}</th>
						<td><input type="text" id="ehr_p_code" name="ehr_p_code" class="w100"></td>
						<th scope="col">${e3ps:getMessage('EHR CODE')}</th>
						<td><input type="text" id="ehr_code" name="ehr_code" class="w100"></td>
					</tr>
					<tr>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<c:if test="${e3ps:isSyncBtn()}">
				<button type="button" class="s_bt03" onclick="sync('syncERPDepartment');">${e3ps:getMessage('동기화')}</button>
			</c:if>
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPDepartment');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
