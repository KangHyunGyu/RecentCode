...
var columnLayout = [
	{ dataField : "ecr_number",		headerText : "${e3ps:getMessage('ECR 번호')}",		width:"11%",
		...
		renderer : {
			...
			jsCallback : function(rowIndex, columnIndex, value, item) {
				...
				var url = getURLString("/erp/viewECRInfo?instance_id="+oid)
				openPopup(url, oid);
			}
		}
	},
	{ dataField : "ecr_name",		headerText : "${e3ps:getMessage('ECR 제목')}",		width:"18%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	...
	{ dataField : "ecr_creator_id",		headerText : "${e3ps:getMessage('ECR 등록자 사번')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "ecr_dept_code",		headerText : "${e3ps:getMessage('ECR 등록 부서 코드')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchECRInfoScrollAction"));
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<td><input type="text" id="ecr_name" name="ecr_name" class="w100"></td>
					</tr>
					<tr>
