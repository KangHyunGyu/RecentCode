...
//AUIGrid 칼럼 설정
var columnLayout = [
	{ dataField : "pjt_type",		headerText : "${e3ps:getMessage('프로젝트 종류')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	...
	{ dataField : "pjt_number",		headerText : "${e3ps:getMessage('프로젝트 번호')}",		width:"13%",
		...
		renderer : {
			...
			jsCallback : function(rowIndex, columnIndex, value, item) {
				...
				var url = getURLString("/erp/viewProjectInfo?instance_id="+oid)
				openPopup(url, oid);
			}
		}
	},
	{ dataField : "pjt_name",		headerText : "${e3ps:getMessage('프로젝트 명')}",		width:"20%", style:"AUIGrid_Left" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "plan_start_date",		headerText : "${e3ps:getMessage('계획 시작 일자')}",		width:"13%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "plan_end_date",		headerText : "${e3ps:getMessage('계획 종료 일자')}",		width:"13%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "process_code",		headerText : "${e3ps:getMessage('공정 코드')}",		width:"10%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pjt_team",		headerText : "${e3ps:getMessage('사업부/팀')}",		width:"10%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pjt_state",		headerText : "${e3ps:getMessage('프로젝트 상태')}",		width:"11%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "description",		headerText : "${e3ps:getMessage('프로젝트 설명')}",		width:"18%", style:"AUIGrid_Left" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pm_id",		headerText : "${e3ps:getMessage('PM 사번')}",		width:"9%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "pm_name",		headerText : "${e3ps:getMessage('PM 이름')}",		width:"9%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "customer",		headerText : "${e3ps:getMessage('거래처')}",		width:"17%", style:"AUIGrid_Left" ,
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "acceptance_date",		headerText : "${e3ps:getMessage('수주일자')}",		width:"10%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "site_code",		headerText : "${e3ps:getMessage('SITE 코드')}",		width:"11%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "fab_name",		headerText : "${e3ps:getMessage('FAB NAME')}",		width:"11%", 
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchProjectInfoScrollAction"));
	...
	<div class="pro_table mr30 ml30">
		<form name="searchForm" id="searchForm">
			<table class="mainTable">
				...
				<tbody>
					<tr>
						<th scope="col">${e3ps:getMessage('프로젝트 종류')}</th>
						<td><input type="text" id="pjt_type" name="pjt_type" class="w100"></td>
					...
					<tr>
						...
						<td><input type="text" id="pjt_name" name="pjt_name" class="w100"></td>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<c:if test="${e3ps:isSyncBtn()}">
				<button type="button" class="s_bt03" onclick="sync('syncERPProject');">${e3ps:getMessage('동기화')}</button>
			</c:if>
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPProject');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
