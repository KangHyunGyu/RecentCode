...
var columnLayout = [
	...
	{ dataField : "ecn_number",		headerText : "${e3ps:getMessage('ECN 번호')}",		width:"11%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
		renderer : {
			...
			jsCallback : function(rowIndex, columnIndex, value, item) {
				...
				var url = getURLString("/erp/viewECNInfo?instance_id="+oid)
				alert(url);
				openPopup(url, oid);
			}
		}
	},
	{ dataField : "ecn_name",		headerText : "${e3ps:getMessage('ECN 제목')}",		width:"18%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "pjt_no",		headerText : "${e3ps:getMessage('프로젝트 번호')}",		width:"13%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "modify_date",		headerText : "${e3ps:getMessage('ECR 등록/수정일')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "ecr_type",		headerText : "${e3ps:getMessage('설계변경 상세 유형')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "ecn_creator_id",		headerText : "${e3ps:getMessage('ECR 등록자 사번')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "ecn_dept_code",		headerText : "${e3ps:getMessage('ECR 등록 부서 코드')}",		width:"14%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
	},
	{ dataField : "ecr_cause",		headerText : "${e3ps:getMessage('변경 사유')}",		width:"15%",
		filter : {
			showIcon : true,
			iconWidth:30
		},
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/erp/searchECNInfoScrollAction"));
...
<div class="product">
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<td colspan="3"><input type="text" id="ecr_number" name="ecr_number" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('ECN 번호')}</th>
						<td><input type="text" id="ecn_number" name="ecn_number" class="w100"></td>
						<th scope="col">${e3ps:getMessage('ECR 제목')}</th>
						<td><input type="text" id="ecn_name" name="ecn_name" class="w100"></td>
					</tr>
					<tr>
						<th scope="col">${e3ps:getMessage('프로젝트 번호')}</th>
						<td><input type="text" id="pjt_no" name="pjt_no" class="w100"></td>
	...
	<div class="seach_arm pt5 pb5">
		...
		<div class="rightbt">
			...
			<img class="pointer mb5" onclick="excelDown('searchForm', 'excelDownERPECR');" src="/Windchill/jsp/portal/icon/fileicon/xls.gif" border="0">
		</div>
	</div>
	<!-- //button -->
	<!-- table list-->
