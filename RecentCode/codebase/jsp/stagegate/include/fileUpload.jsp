...
// 	}
	...
	String masterHost = ConfigImpl.getInstance().getString("HTTP.HOST.URL")+"/Windchill/";
	String userId =  wtUser.getName();
	String upload_fullpath = multi.getParameter("upload_fullpath");
	String roleType = multi.getParameter("roleType");
	String originFileName = upload_fullpath;//multi.getOriginalFileName(roleType);
	String fileName = upload_fullpath;//multi.getFilesystemName(roleType);
	String fileExtType = FilenameUtils.getExtension(fileName); //multi.getContentType(roleType);
...
// 	System.out.println("################5 fileName : "+fileName +" , length = " +fileName.length());
	...
	if(file == null) {
		throw new Exception(MessageUtil.getMessage("복호화 불가능한 파일입니다."));
	}
