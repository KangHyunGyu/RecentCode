...
function modifyDistribute() {
	var url = getURLString("/distribute/modifyDistributeRegistration") + "?oid=${oid}";
	location.href = url;
}
...
function deleteDistribute() {
	openConfirm("${e3ps:getMessage('삭제하시겠습니까?')}", function(){
		...
		var url = getURLString("/distribute/deleteDistributeRegistrationAction");
...
<div class="pop">
	...
	<div class="tap pt20">
		<ul>
			<li onclick="loadIncludePage(this);" data-url="${e3ps:getURLString('/distribute/include_detailDistributeRegistration')}">${e3ps:getMessage('상세보기')}</li>
