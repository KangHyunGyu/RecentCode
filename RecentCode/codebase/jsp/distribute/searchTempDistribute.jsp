...
webix.ready(function(){
	var grid = webix.ui({
		...
		datafetch:50,
		columns:[
			{ id : "tempNumber",		header : "${e3ps:getMessage('배포 번호')}",		width:"140",	
				css: "custom_webix_ellipsis",
				template:function(obj){
					var oid = obj.oid;
					return "<a href='#' onclick='openView(\""+oid+"\")'>"+obj.tempNumber+"</a>";
				},
			},
			{ id : "title",		header : "${e3ps:getMessage('배포 제목')}",		width:"140", fillspace: true,		
				css: "custom_webix_ellipsis",
			},
			{ id : "pjtNo",		header : "${e3ps:getMessage('프로젝트 번호')}",		width:"140",	
				css: "custom_webix_ellipsis custom_webix_center",
			},
			{ id : "createDateFormat",		header : "${e3ps:getMessage('작성일')}",		width:"140",	
				css: "custom_webix_ellipsis custom_webix_center",
			},
			{ id : "creator",		header : "${e3ps:getMessage('작성자')}",		width:"140",	
				css: "custom_webix_ellipsis custom_webix_center",
			},
		],
		on:{
...
function getGridData(){
	$$("grid_wrap").load(function(params){
		...
		return webix.ajax().headers({
		    ...
		    }).post(getURLString("/distribute/searchTempDistributeScrollAction"), params)
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						...
						<td><input type="text" id="tempNumber" name="tempNumber" class="w100"></td>
