...
function save(btn) {
	...
	try{
		...
		if(isEmpty(getElementValue('distName'))){
			throw new Error("${e3ps:getMessage('의뢰서 명을 작성해주세요')}");
...
// 	}
	...
	if(param.approvalList)param.approvalList = approvalList;
...
// 	$("#distributeForm").attr("action",getURLString("/distribute/requestFormDistributeAction"));
// 	formSubmit("distributeForm", param, msg, null, true);
	
	var url	= getURLString("/distribute/requestFormDistributeAction");
