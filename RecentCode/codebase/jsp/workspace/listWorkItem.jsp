	...
	{ dataField : "roleName",		headerText : "${e3ps:getMessage('활동')}",		width:"10%", style:"editAble_Blue_Cell",
		...
		renderer : {
			...
			jsCallback : function(rowIndex, columnIndex, value, item) {
				...
				var url = "";
				if("PROJECT_ISSUE" == objectType){
					var objOid = item.objectOid;
					openView(objOid);
				}else{
					url = getURLString("/workspace/detailApproval") + "?type=${type}&oid=" + oid;
					location.href = url;
				}
			}
		}
...
function getGridData(){
	...
	var url = getURLString("/workspace/listWorkItemAction");
	...
	*/
	$("#searchForm").attr("action", getURLString("/workspace/listWorkItemAction"));
	param["type"] = "${type}";
	AUIGrid.showAjaxLoader(myGridID);
