...
var columnLayout = [
	/*
	{ dataField : "lineReceiveCheck",		headerText : "${e3ps:getMessage('수신여부')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
     ...
     */              
	{ dataField : "objectTypeName",		headerText : "${e3ps:getMessage('구분')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}
	...
	{ dataField : "createDateFormat",		headerText : "${e3ps:getMessage('등록일')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	},
	{ dataField : "completeDateFormat",		headerText : "${e3ps:getMessage('완료일')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
	...
	{ dataField : "createFullName",		headerText : "${e3ps:getMessage('등록자')}",		width:"8%",
		filter : {
			showIcon : true,
			iconWidth:30
		}	
...
function getGridData(){
	...
	$("#searchForm").attr("action", getURLString("/workspace/listItemScrollAction"));
	...
	param["page"] = page;
	param["rows"] = rows;
	//param["reciveCheck"] = $("#reciveCheck").val();
	
	//수신 여부 체크 
	var receivelist = new Array();
	$('input:checkbox[name="reciveCheck"]').each(function() {
		if(this.checked){
			var val = $(this).val();
			receivelist.push(val);;
		}
		
	});
	param["reciveCheck"] = receivelist
	...
	<div class="pro_table mr30 ml30">
		<form name="searchForm" id="searchForm">
			...
			<table class="mainTable">
				...
				<tbody>
					<tr>
						<th scope="col">${e3ps:getMessage('결재 구분')}</th>
						<td>
							<div class="pro_view">
